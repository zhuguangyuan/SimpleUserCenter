...
public class ReadyPay{
	...
	public static void main(String[] args) throws Exception {
		...
		DateArry.put("txn_amt", "10");//交易金额 [单位：分  例：1元则提交100]，此处注意数据类型的转转，建议使用BigDecimal类弄进行转换为字串
		 ...
		 */
		 ...
		 if(ReturnData.get("resp_code").toString().equals("S")){
			...
			Log.Write("预支付唯一码:"+SecurityUtil.Base64Decode(SecurityUtil.AesDecrypt(ReturnData.get("unique_code"),RAesKey)));
		 }else{
			 Log.Write("失败！");
		 }
	}
